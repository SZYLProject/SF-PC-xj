...
import { SetDevCtrl,General,RecordFile,DevInfo,OpenDevice,CloseDevice } from "./deviceapi.js";
import { AppendStatusEx } from "./pstnIndex";
...
function TV_HangUpCtrl(uID) {
	// TV_EnableHook(uID, FALSE);//by old
	TV_EnableHook(uID, 0); // 挂机 by yuwen
	TV_StopRecordFile(0); // 结束录音by yuwen
}
//----------------------------------------
...
function TV_StartPlayFile(uID, szFile) {
	var curentTimeVal =curentTime();
	vFilePath = "c:\\录音"+curentTimeVal+".wav";
...
function TV_StartRecordFile(uID,val2,patientName) {
	var curentTimeVal =curentTime();
	console.log(patientName,'patientName')
	if(patientName ==undefined|| patientName=="undefined"||!patientName){
		patientName =""
	}
	var vFilePath =  "c:\\"+patientName+curentTimeVal+".wav";
	// var vFilePath = "c:\\test.wav";  //有些操作系统c盘保护，保存到c盘可能会失败，可以换到其它盘符
...
export {
	...
	TV_Disable, //关闭设备
	TV_EnableMic,  //mic
	TV_EnableLine2Spk, // 耳机
	TV_StartDial, // 拨号
	TV_HangUpCtrl, // 挂机
	TV_StartRecordFile, //开始录音
	TV_StopRecordFile, // 结束录音
}