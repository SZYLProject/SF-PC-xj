...
<template>
  <section class="module-advanced">
      ...
      <ul>
        ...
        <li v-for="(item, index) in searchForm.conditionItemList" :key="index">
          <ol>
            ...
            <li>
              <el-select size="mini" v-model="item.conditionVal" placeholder="关系">
                <el-option
          ...
          <ol>
            <li v-for="(filterItem, filterIndex) in item.filterList" :key="filterIndex">
              <dl>
                ...
                <dd>
                  <el-select size="mini" v-model="filterItem.conditionVal" placeholder="关系">
                    <el-option
    ...
    <section class="date-count">
      <el-checkbox v-model="searchForm.checked">
        去除有就诊记录的患者,去除天数:
        <el-input
          type="number"
          size="mini"
          class="input-dateCount"
          v-model="searchForm.dateCount"
          style="width:100px;"
        ></el-input>
      </el-checkbox>
    </section>
...
import { getFindCondition } from "@/api/survivalFollowup";
export default {
  ...
  methods: {
    ...
    getFindConditionFn() {
      ...
      getFindCondition(params)
        .then((res) => {
          let backRestaurants = res.data.map((item) => {
            return { value: item.column_comment, name: item.column_name };
          });
          this.searchForm.conditionItemList[0].restaurants = backRestaurants;
        })
        .catch((error) => {
          console.error(error);
        });
    ...
    //搜索
    handleSearch() {
      Bus.$emit("searchDataFormGj", this.searchForm);
