  ...
  <div class style="height: calc(100% - 36px); background-color: #ffffff">
    <div id="visitPlan">
      ...
      <div class="text-center margintb24">
        ...
        <el-button type="primary" @click="savePlan">保存当前阶段</el-button>
      ...
      >
        <el-form :model="dataVisit">
          <el-form-item class="label100" label="模板名称">
            <el-input
              v-model="dataVisit.name"
              placeholder="请输入访视名称"
            ></el-input>
          </el-form-item>
          <el-form-item class="label100" label="所属科室">
            <el-select
...
import {
  registrationSaveNextStep, // 保存访视计划
  registrationGetBeanById, // 查询访视计划
  getSmsList,
  getWeChatList,
...
export default {
  ...
  created() {
    ...
    this.getFormListFn(); //获取表单模板
    // this.getDepartmentsFn();

    this.registrationGetBeanByIdFn(); //获取计划列表数据
  },
  methods: {
    // 获取计划列表数据
    registrationGetBeanByIdFn() {
      let getdata = {
        id: this.projectId,
      };
      registrationGetBeanById(getdata)
        .then((res) => {
          ...
          localStorage.setItem(
            "searchConditionTumor",
            res.data.searchCondition || ""
          );
          let dataBack = res.data.nodeList;
          dataBack.length === 0
    ...
    onConfirm() {
      const postData = {
        ...
        departmentId: this.dataVisit.department[1],
      ...
      //“name”: "模板名称", //模板名称
      // “departmentId”:"12" //部门ID
      // “departmentName”:“dd” //部门名称
    ...
    savePlan(val) {
      ...
      this.backPlanId = this.leftGradation[this.current]["planId"];
      postData.projectId = this.projectId;
      postData.planId = this.backPlanId;
      postData.nodeId = this.backNodeId;
      postData.nodeName = this.leftGradation[this.current]["nodeName"]; //阶段name
      registrationSaveNextStep(postData).then((res) => {
        this.$message({
          showClose: true,
          message: "保存成功",
          type: "success",
        ...
        this.$nextTick(() => {
          this.registrationGetBeanByIdFn();
        });
        if (val == "next") {
    ...
    //获取微信模板
    getWeChatListFn() {
      getWeChatList({ typeCode: "registration" })
        .then((res) => {
    ...
    //获取短信模板
    getSmsListFn() {
      getSmsList({ typeCode: "registration" })
        .then((res) => {
    ...
    getFormListFn() {
      getFormList()
        .then((res) => {
          // this.formList = res.data.dataList.reverse();
          // this.formData.formId = [this.formList[0]["pkId"]];
