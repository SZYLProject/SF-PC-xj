<template>
  <div class style="height: calc(100% - 36px); background-color: #ffffff">
    <div id="visitPlan">
      <!-- 中侧随访计划列表 -->
      <ul class="plan-gradation" v-if="followType == 'prospect'">
          ...
          <div v-if="followType == 'prospect'">
            ...
            <el-form-item>
              ...
              >
                <ul>
                  <li
                    v-for="(item, index) in formData.cycleRegular"
                    :key="index"
                  >
...
<script>
import { parseTime, isArray, isObj, getLength, Compare } from "@/utils/index.js";
import {
  getTemplateListByTypeCode,
  addPlan,
  updatePlan,
  updatePlanName,
  deletePlan,
...
export default {
  ...
  created() {
    this.getWeChatListFn(); //获取微信模板
    this.getSmsListFn(); //获取短信模板
    this.getTemplateListByTypeCodeFn(); //获取表单模板
    // this.getDepartmentsFn();

    this.getLiveBeanByIdFn(); //获取计划列表数据
    // getLiveBeanById
  },
  methods: {
    // 获取计划列表数据
    getLiveBeanByIdFn() {
      ...
      getLiveBeanById(getdata)
        .then((res) => {
          localStorage.setItem("searchCondition", res.data.searchCondition);
          // let dataBack =  Object.assign([],res.data.nodeList);
          let dataBack = JSON.parse(JSON.stringify(res.data.nodeList));
          dataBack.length === 0
    ...
    savePlan(val) {
      this.$refs.formData.validate((valid) => {
        if (valid) {
          ...
          this.backNodeId = this.leftGradation[this.current]["id"];
          postData.projectId = this.$route.query.id;
          postData.planId = this.backPlanId;
          postData.nodeId = this.backNodeId;
          ...
          postSaveNextStep(postData).then((res) => {
            this.$message({
              showClose: true,
              message: "阶段保存成功",
              type: "success",
            ...
            this.backPlanId = res.data.planId;
            this.$nextTick(() => {
              this.getLiveBeanByIdFn();
            });
            if (val == "next") {
              this.$emit("childNextStep", 2);
            }
          });
        } else {
    ...
    turnActive() {
      ...
      } else if (this.followType == "prospect") {
        this.savePlan("next");
      }
    },
    //中侧计划列表添加阶段
    ...
    //获取微信模板
    getWeChatListFn() {
      getWeChatList({typeCode:'survival'})
        .then((res) => {
    ...
    //获取表单模板
    getTemplateListByTypeCodeFn() {
      let getData={
        typeCode:"survival"
      }
      getTemplateListByTypeCode(getData)
        .then((res) => {
        ...
        .catch((error) => {
          console.error("getTemplateListByTypeCode", error);
        });
    },
    // 切换阶段
    currentTab(index) {
      ...
      if((state || !result && this.leftGradation[this.current].id!=this.formData.id)){
