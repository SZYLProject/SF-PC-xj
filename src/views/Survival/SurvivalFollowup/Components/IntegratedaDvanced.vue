...
// import { specialFindPatientInfoByCondition } from "@/api/specialFollowUp";
import {
  getFindTableAndColumn,
  getFindColumns,
  gitFindTabl,
...
export default {
  ...
  created() {
    ...
    console.log(
      ...
      "this.searchCondition.conditionItemList,33333333333333333333"
    );
    if (this.searchCondition) {
      //  高级检索
      // this.advancedSearch.conditionItemList = this.searchCondition.conditionItemList;
      console.log(
        this.searchCondition.conditionItemList,
        "this.searchCondition.conditionItemList,222222222222222222222222222222222222"
      );
      if (this.searchCondition.conditionItemList.length!=0) {
        this.searchForm.conditionItemList =
          this.searchCondition.conditionItemList;
      } else {
            this.searchCondition.conditionItemList = [
              {
  ...
  methods: {
    ...
    // 获取第一层筛选条件
    getFindTableAndColumnFn() {
      let params = {
        // tableName: "t_data_patient",
      };
      getFindTableAndColumn(params)
        .then((res) => {
    ...
    // 获取第二层筛选条件
    getFindColumnsFn(val) {
      let params = {
        name: val,
      };
      getFindColumns(params)
        .then((res) => {
    ...
    handleAddFilterCon(index) {
      ...
      if (superiorVal) {
        this.getFindColumnsFn(superiorVal);
    ...
    //搜索
    handleSearch() {
      console.log(this.searchForm, "2222222222222222");
      return this.searchForm;
      // Bus.$emit("searchDataForm", this.searchForm);
      // console.log(JSON.stringify(this.searchForm), "3333");
