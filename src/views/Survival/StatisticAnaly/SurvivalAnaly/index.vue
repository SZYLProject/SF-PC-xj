<template>
  <div class="SurvivalAnalyContent content">
      ...
      >
        ...
        <el-form-item class="buttons">
          <el-button
            size="mini"
            style="width: 70px"
            @click="resetForm('SurvivalAnalyForm')"
            >重置</el-button
          >
          <el-button
            size="mini"
            style="width: 70px"
            type="primary"
            @click="submitForm('SurvivalAnalyForm')"
            >查询</el-button
          >
          <el-button
            size="mini"
    ...
    <div class="query-result">
      ...
      <div>
        <div
          id="diseaseLineEchart"
          ref="diseaseLineEchart"
          class="data_view-container"
          v-loading="diseaseLineLoading"
        ></div>
      </div>
    </div>
...
import {
  ...
  getP433,
  getP434,
  SurvivalConditionAnalysis,
  SurvivalConditionAnalysisKM,
  exportBySur,
  exportByKM,
  lifeTableMethod,
...
export default {
  ...
  created() {
    ...
    this.diseaseLineListFn();
    // this.getInfoByDiseaseFn()
  },
  methods: {
    diseaseLineSearch() {},
    // 人员查询图表初始化 ( 有效随访人次 )
    staffInitValid(dataX, dataY, dataLegend) {
      this.$nextTick(() => {
        let _this = this;
        let staffChart = document.getElementById("diseaseLineEchart");
    ...
    SurvivalConditionAnalysisFn() {
      console.log(this.SurvivalAnalyForm);
      const postData = {
    ...
    exportFormAssist(res) {
      ...
      downloadElement.download =
        "生存随访生存情况分析生存率" +
        (this.activeName === "first" ? "K-M算法" : "寿命表法") +
        timeNow +
        ".xlsx"; // 下载后文件名
    ...
    //  get K-M病种
    diseaseLineListFn() {
      getFindDiseases().then((res) => {
        this.diseaseLineList = res.data.reduce((t, e, i) => {
          t.push({ name: e, value: e });
          return t;
        }, []);
        this.diseaseLine = [this.diseaseLineList[0].value];
        this.getInfoByDiseaseFn();
        console.log(res, " get 病种");
        // this.getlifeTableMethod();
      });
    },
    getInfoByDiseaseFn() {
      let postData = {
        diseaseList: this.diseaseLine,
      };
      let dataLegend = [];
      this.diseaseLineLoading = true;

      getInfoByDisease(postData).then((res) => {
        this.diseaseLineLoading = false;

        let dataX = res.data[0].X.map((x) => {
          //  return parseInt(x);
          return Number(x) == parseInt(x) ? parseInt(x) : x;
        });
        // let yData = res.data[0].Y;
        // yData = yData.map((x) => {
        //   return x * 100;
        // });
        let dataY = res.data.reduce((total, item, index) => {
          dataLegend.push(item.diseases);
          let yData = item.Y;
          yData = yData.map((x) => {
            return Math.round(parseFloat(x * 100) * 100) / 100;
          });
        ...
        this.staffInitValid(dataX, dataY, dataLegend);

        console.log(res, " get 病种折线");
        // this.getlifeTableMethod();
      });
    },
    // get医院科室
