      ...
      <ul>
        ...
        <li v-for="(item, index) in searchForm.conditionItemList" :key="index">
          <ol>
            ...
            <li>
              <!-- :fetch-suggestions="querySearch" -->
              <el-autocomplete v-model="item.range" placeholder="对比值">
                <i slot="suffix"></i>
                <template slot-scope="{ item }">
                  <div class="name">{{ item.value }}</div>
                </template>
...
import { getFindCondition } from "@/api/specialFollowUp";
  ...
  methods: {
    ...
    getFindConditionFn() {
      ...
      getFindCondition(params)
        .then((res) => {
          console.log(res, "columnComment", this.searchForm.conditionItemList);
          // res.data = res.data.map((item) => {
          //   return { value: item.columnComment, name: item.columnName };
          // });

          this.searchForm.conditionItemList[0].restaurants = res.data;
        })
        .catch((error) => {
          console.error(error);
        });
